...
| In-app object-storage deploy (rclone driven by the desktop shell, keys kept local) — the last SITE destination whose delivery ends in a terminal *(tend re-run 2026-08-22, `32bf722`; ledgers/TEND-rerun-2026-08-22.md O4-seam)* | **handed-to-graft: V2/Q-15 vantage** — no latent half exists; a transport enforcer would be new code beside `src-tauri/src/github/`. Tend's reachable slice (copy/onboarding reach) is ticketed as Archie-848c |
