...
The Tauri v2 shell (`src-tauri/`) bundles Studio+Viewer into a native window; the ONLY app-side
Tauri touchpoint is `apps/studio/src/tauri-fs.ts` (dynamic-imports `@tauri-apps/*`, gated by
...
- Archie-2139 — "Open my site" and every external link were silently dead (opener scope covered
  only the login URL); `openExternal` now pins hostnames itself, since the capability glob can't / 4730681
...
- Archie-623e — native folder is becoming the canonical desktop store (OPFS demoted to web-only);
  **still OPEN**, all 6 phases code-landed (`8140bc4`) but blocked on packaged proof, not on code

## Evidence
- `docs/plans/native-canonical-store.md` §"a09d packaged verification" — 7-row checklist, every row
...
- `ledgers/TEND-EXPLORE-tauri-2026-07-20.md` — dated exploration; **its L1/L2 "OPFS still canonical"
  ...
  metainfo screenshots) were fixed same week — see Decisions above, not this file, for current truth.

## Open & hazards
- `src-tauri/README.md:44-48` still calls asset-protocol/`convertFileSrc` "the real remaining
  work" / "highest-risk item" — **doc is stale**, Phase 4 (`convertFileSrc` for AV, `ff79829`)
  shipped. Don't cite the README for current state; cite `docs/plans/native-canonical-store.md`.
- Archie-9ece (open, blocks Archie-623e) — nothing in the 7-row checklist may be claimed "done" on
