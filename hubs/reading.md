...
`ExhibitView.svelte` (mounts the four readers behind lazy imports) and `astro.config.mjs`
...
- Archie-7b86 — AV reading surface: baked waveform peaks (V50), note surface restored (V53), temporal
...
## Open & hazards
- No eager-bytes ratchet on the viewer's exhibit route — the 7.8x lazy-split win can regress silently
  the same way the embed's did before `eagerGzKB` existed (`[[archie-viewer-eager-closure]]` is the
  embed's version; nothing analogous watches `apps/viewer`)
- Video poster-at-ingest (kill the live `<video preload="metadata">` per gallery card, 5.9x on routes
  with AV) and TTF→WOFF2 brand fonts (~60KB, no tradeoff) are both identified, unshipped
