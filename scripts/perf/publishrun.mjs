// Runs scripts/perf/publishbench.ts (END-TO-END publish) in real Chromium.
//
...
const server = await createServer({
  ...
  server: { port: 5396, strictPort: true, fs: { allow: [REPO] } },
});
await server.listen();
console.log("• bench server http://localhost:5396/publish.html");
...
await page.goto("http://localhost:5396/publish.html", { waitUntil: "load" });
console.log("• running…\n");
