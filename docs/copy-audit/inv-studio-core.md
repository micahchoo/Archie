...
| apps/studio/src/NarrativeEditor.svelte | — | ~~`Move here`~~ **[SUPERSEDED 2026-06-19]** — button REMOVED; the "Shown with · [object]" line is now the card's `.go-to` navigation control (jump to the section's object + focus its frame). New title/aria-label: `Go to [object] and show this section's framed view` (off-object) · `Center the canvas on this section's framed view` (on-object). | resolved — audited string no longer exists | — | done | internal-vocab |
