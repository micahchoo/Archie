...
- *Pixel selector is still derived in Phase 1 — nothing populates `geo` yet.* That wiring is **Phase 2**.

**Status — 2026-06-18 (Phase 2: DONE; Phase 4: DONE).** Decision: **Option A** (whole-world tiles + viewport pan-constraint; world-pixel annotations; R8-free).
- *2b — viewport constraint: DONE* (`mount.ts` fit-to-region + settle-time zoom-floor/pan clamp; **browser-verify-owed** for OSD runtime). *2d — reader coordinate display: DONE* (`geoCenter` + the opened-note coordinate line in Reader/NarrativeReader, Q7; attribution + locator already shipped). *Phase 4 — manifest emit/readback: DONE* (`archie:tileSource` round-trips toCanvas↔objectsFromManifest; published maps render in the Viewer). *Phase 3 — add-map modal: DONE* (`apps/studio/src/AddMapModal.svelte` + `+ Map` trigger + `addMapObject`). Curated provider list (OSM/Carto — embedding-compliant, attribution baked in → D6); extent via region presets + bounds fields + a Max-zoom; "advanced: custom tile URL" escape; emits a `tileSource` descriptor → new Map Object. Studio builds. **human-gate owed** (comprehension: does an author grok "pick a basemap, set the region?"). **Graphical clamping DONE** (2026-06-18): the modal renders a world-tile locator — drag to draw a region, drag corner handles to clamp; two-way synced with the bounds fields (Q3 extent gesture delivered). **Map thumbnail DONE**: `thumbSrc` passes the descriptor to `thumbnailUrl` → the z0 world tile (rail/overview no longer broken; region-centred thumbnail is a possible refinement). Remaining overall: **Phase 5** (offline tiles — excluded by current goal).
- *2a — pin removal: DONE* (`c7148b0`). DrawTool drops `pin`; the synthesized canvas-click gesture removed from `mount.ts`; the 📍 Pin button + pin copy removed from Studio. Geo-Notes are Box/Outline only.
- *2c — geo-truth capture on draw: DONE* (this commit). Studio `onCreate`/`onUpdate` compute `geoForTarget` (Box→bbox via NW/SE corners; Outline→polygon per vertex) from the WORLD-pixel selector and persist `geo` (truth); the seeded geo-map carries `geo`; the readout (`geoLabelOf`) prefers stored truth, falls back to pixel-derivation. Verified: studio build, viewer astro-check 0 errors.
- *REMAINING in Phase 2:* **2b** — the OSD viewport pan-constraint to `regionPixelRect` (Option A; mount.ts) — **browser-verify-owed** (OSD runtime, not headless-testable; drive with lightpanda/reliable-test-loop). **2d** — reader coordinate display in the opened note (Viewer, Q7) [attribution + locator already shipped in the prototype]. The Q8 re-frame warning waits for bounds-editing UI (Phase 3 modal).
