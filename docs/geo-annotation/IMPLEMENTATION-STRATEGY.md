...
- *2b — viewport constraint: DONE* (`mount.ts` fit-to-region + settle-time zoom-floor/pan clamp; **browser-verify-owed** for OSD runtime). *2d — reader coordinate display: DONE* (`geoCenter` + the opened-note coordinate line in Reader/NarrativeReader, Q7; attribution + locator already shipped). *Phase 4 — manifest emit/readback: DONE* (`archie:tileSource` round-trips toCanvas↔objectsFromManifest; published maps render in the Viewer). Remaining: **Phase 3** (add-map modal — invented UX, human-gated) and **Phase 5** (offline tiles — excluded by current goal).
