...
**Status — 2026-06-18 (Phase 1, part 1 of 2: DONE).** The bounded coordinate space is built + verified: `tileSource.bounds`, `regionPixelRect`, `tileRangeForBounds`, `geoInBounds` (`render-core/geometry/geo.ts`), R8 corpus green (`geo.test.ts`, 21 tests; render-core 512). **R8 dissolved** — the realization keeps the pixel space bounds-independent (world pixels) and treats the extent as a rectangle, so the projection/tile-URL math stay the verified whole-world code (see ADR-0015 Update). **Remaining Phase-1 (part 2): geo-truth persistence** — `GeoAnchor = bbox | polygon`, thread `archie:geo` (lng/lat = truth) through `NewNote → AnnotationRecord → serialize` (ARCHIE_EMPHASIS pattern), derive the pixel selector on render. **Phase-2 entry fork (A/B):** the OSD bounding mechanism — (A) whole-world tiles + viewport pan-constraint, world-pixel annotations (recommended, R8-free) vs (B) region-local raster + offset remap (re-introduces R8). Reversible under geo-truth.
