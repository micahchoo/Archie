...
| Q-6 | Preview hands the built tree to the embed IN-PROCESS; the hosted deep-link is the post-publish button | 2026-07-26 | - | **ACCEPTED — user-gated 2026-07-26.** Studio never mints a URL for preview: the desktop CSP allows `connect-src https:` but not `blob:`, so a served-tree preview would need the CSP relaxed. `publish-flows.previewTree()` reuses `projectSite(false)` and hands the MemoryFilesystem to `<archie-viewer>.openLibraryFs`. Enforced end-to-end by `apps/studio/e2e/preview.spec.ts`, which asserts the reader reaches its gallery AND that the gallery carries the exhibit (the shell alone paints either way — element.ts:816). |
<!-- DECISIONS_INDEX_END -->
