...
| note media (images in notes) | `NoteMedia`, `NoteLightbox` | `note-card.ts` (tiles on the card, figures + visible caption in the sheet) | ADAPT (tiles and sheet; no separate lightbox — enlarge is the sheet, so the embed has ONE overlay and the shell's "a modal replaces the sheet, never stacks" rule has nothing to arbitrate) | **MUST** | smoke — the tile's `aria-label` is compared against the DESCRIPTION in the published annotation's own body, the prose is asserted to contain no raw `<img>`, and the sheet's `figcaption` and figure `src` are compared against that same body |
| reading sheet (long-form reading text) | `ReadingSheet.svelte` | `note-card.ts` (⤢ → an in-element modal, ONE renderer shared with the card) | ADAPT (Archie-dbbc's shape: one content renderer, card hidden-not-unmounted; the sheet is `position:absolute` within the element, never `fixed` — an embed must stay in its own box, and the shell's portal-to-`<body>` is unavailable behind a shadow root) | **MUST** | smoke — a REAL driven `page.mouse.click()` on the ⤢ (reporting what `elementFromPoint` returned there), then: the sheet is `role=dialog aria-modal=true` and covers the element, carries BOTH prose and media (V60), the card is still MOUNTED and hidden (V62/V63), and Escape restores focus to the ⤢ (V63) |
| cite hovercards in prose | `ProseCites.svelte` | — | **DROP-justified** — the payload does not survive the port, and the mechanism cannot. `CiteCard.svelte:19-40` resolves a note cite to the link's own label, a kind badge, and the EXHIBIT's title and cover — never the note; `crop` is forward-compat wiring and "currently always undefined" (`:7-9`), so a region cite is a badge over an empty box. The card is `position:fixed` and must portal to `<body>` to escape a transformed ancestor (`ProseCites.svelte:76-82`) — inside a shadow root that means leaving the embed's own token scope, and inside a host iframe it is clipped regardless. Hover is also pointer-only, and an embed is the most touch-heavy surface Archie ships. What remains is a floating box repeating a word the reader just read, plus the exhibit's name | — | none — and see the finding below: no published fixture carries a cite at all, so this row could not have been gated even if built |
| full-text search | `SearchOverlay.svelte` + minisearch | `reader-chrome.ts` `searchExhibit` + the pane's `.rc-find` filter (lazy) | **DONE-differently** — the weight objection dissolved rather than being paid. minisearch exists because the shell's finder is a library-scale browse surface; the embed already holds the exhibit's whole note tree in memory when the exhibit opens, so an exact substring scan over it needs NO index and NO dependency (`eagerGzKB` unmoved at 39.1, +0.7 KB lazy). It is a filter over the pane's existing note index rather than a modal over the canvas — no scrim, no second focus trap, and it never covers the image. Prefix/fuzzy deliberately not ported: an exact match is a promise the embed can keep | **MUST** | smoke — the hit count is compared against the notes carrying the query in the PUBLISHED tree (12 across 8 objects, where the open object held 2), EVERY hit is asserted to name its object, and a driven click on a hit elsewhere must land with the note CARD OPEN, not merely on the right object |
...
- A `DEFER-tracked` row without a ticket is a half-measure. **There are now no `DEFER-tracked` rows.**
  The four that existed were resolved together by `Archie-1820` (2026-07-26) — three built, one
  dropped with its reason written into the row itself. The ledger is
  `ledgers/FIX-embed-defer-2026-07-27.md`.
- **One of those rows was wrong about the code, in the direction that hides work.** It recorded the
  embed as *dropping* note media. It did not: `renderMarkdown` passed `![alt](url)` through to
  DOMPurify, so the card rendered an unconstrained remote image at natural width with the author's
  description reachable only as an `alt` attribute. A row describing a capability as ABSENT when it is
  really PRESENT-AND-UNMANAGED points at the wrong fix, and nothing in the table can catch that — the
  verdict vocabulary describes intent, and only reading the code checks it. When a row claims an
  absence, open the file before believing it.
