...
| **"Move here" re-anchors a beat** ("change a section's object") | a **mode transition that must clear stale framing** — moving a beat to the current object re-anchors `objectId` AND must clear/invalidate the beat's framed `start` (an area drawn on the old object's image is meaningless on the new one); ordering preserved; the dimmed→lit state updates | d/a | move-clears-area (framed beat → Move here → `start` cleared, banner re-prompts framing), move-preserves-order, move-updates-dim-state |
