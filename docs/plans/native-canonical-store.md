...
| P5-b | 5 | The on-disk generation-token guard pauses+warns when an out-of-band writer (sync tool) touched the folder. | ☐ pending build — token guard IMPLEMENTED (`resident-store.ts` baseline/`residentExternallyChanged`/`restampResident`, reusing mirror-stamp.ts; `saveLibraryMeta` throws `ResidentExternalChangeError` rather than blind-overwrite). Packaged detection + the richer overwrite/reopen banner UX are packaged-only. |
