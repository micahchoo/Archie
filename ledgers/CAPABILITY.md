...
## Direction 9 — embed-parity: the `<archie-viewer>` embed loads layers it never renders

The embed opens the FULL `PortableExhibit` via the same `readExhibitTree`
(`packages/render-core/src/publish/read.ts:102` returns
`{ slug, title, objects, annotationsByObject, readings, readingAnnotationsByObject, sections, canvasIdByObject, …rightsFromIIIF, summary }`)
as the standalone apps, but `packages/archie-viewer/src/element.ts` accesses only four of those
fields — `objects` (:319,:530), `annotationsByObject` (:344,:389,:531), `canvasIdByObject`
(:345,:451) and `title` (:535,:558). Every other loaded field is carried into memory and dropped at
presentation. Full field-access inventory taken this pass (`grep -oE '(_?exhibit)\.[a-zA-Z]+'` over
element.ts → exactly those four). `slug` is used only for gallery-card navigation (`btn.dataset`), not
rendered per-exhibit; not counted as a drop.

| operation | defined at | user path | gate | intent | class | verdict | commissioned as |
|---|---|---|---|---|---|---|---|
| Render the narrative spine (`exhibit.sections`) — section stepper / narrative reader | Loaded: `read.ts:63,102` (`sectionsFromManifest`). Dropped: `element.ts` never reads `exhibit.sections` (0 accesses); `#renderExhibit` (:529-560) paints a flat object grid regardless of spine | Embed a narrative-authored exhibit via `<archie-viewer src=…>` | None — silently flattened to an object grid; no error, no "this exhibit has a narrative" hint | ADR-0019:9-10 justifies the embed *because* Mirador/UV "cannot render Readings (ADR-0007), the narrative Spine (ADR-0005)"; yet ADRs 0019-0022 never scope the spine out — "read-only" (0019:24, commit 77e5a29) means no-editing, not no-narrative. `SCALE.md:22-27` flags "`<archie-viewer>` embed parity port (flagged out of scope, spike-0005)" as a manual remainder. Designed-minimal-v1 drifting toward forgotten | Surplus (L4→L2: fully loaded, silently unrendered) | **pursue** (user, 2026-07-17 — one parity spec covering all four rows) | Spec interview (shared by all four rows). **Prompt:** "Spec `<archie-viewer>` embed parity: render the narrative spine (`exhibit.sections`), reading layers (`readings` + `readingAnnotationsByObject`, with a minimal reading picker), the exhibit `summary`, and rights/`requiredStatement` attribution — the attribution line is the non-negotiable piece (IIIF must-display) and ships regardless of how the rest is scoped. Constraints: ADR-0019's no-eval boundary holds (build on `createReadOnlyMount`; note ISSUES.md Direction 5's four missing surface methods); the embed is minimal chrome — interview which layout families render in-embed vs get a 'view full exhibit' escape hatch. Sequencing: ISSUES.md Issue 17 (whole-object-note reachability in the embed) rides the same spec, as
does the embed's partial-load indicator (the read layer already sets `exhibit.incomplete` — see
ledgers/READPOLICY.md HANDOFF — only the render is owed). Deliverable: a scoped spec + phasing (attribution-first vs one release); build waits for the Issue 19/20 gate to clear." |
| Render multiple reading layers (`exhibit.readings` + `exhibit.readingAnnotationsByObject`) | Loaded: `read.ts:64,88-94,102`. Dropped: `element.ts` reads only base `annotationsByObject` (:344,:389,:531); `readings` and `readingAnnotationsByObject` have 0 accesses | Embed an exhibit authored with 2+ readings | None — only the base layer's notes show; no reading picker, no hint other layers exist | Same as the spine row: ADR-0019:9-10 names Readings as a first-class reason to embed Archie's own viewer; no scope-out in ADRs 0019-0022; SCALE.md parity-port remainder is the only deferral marker | Surplus (L4→L2) | **pursue** (user, 2026-07-17) | Covered by the parity spec — see the sections row's prompt (reading picker is in scope there). |
| Render the exhibit `summary` (description/caption) | Loaded: `read.ts:102` (spread onto `PublishedExhibitData`, `site.ts:621`). Dropped: `#renderExhibit` header shows only `exhibit.title` (:535); `summary` has 0 accesses | Embed any exhibit that carries a description | None | Not named in any ADR or the SCALE.md note; incidental drop alongside the title-only reader shell. Lower-stakes than the authored layers | Surplus (minor) | **pursue** (user, 2026-07-17 — folded into parity) | Covered by the parity spec — see the sections row's prompt. |
| Display IIIF attribution (`requiredStatement`) + license (`rights`) | Loaded: `read.ts:102` (`…rightsFromIIIF(manifest)` → `RightsFields` on `PublishedExhibitData`, `site.ts:618`). Dropped: `element.ts` never reads `rights`/`requiredStatement` (0 accesses) | Embed an exhibit carrying an attribution credit / license | None — the credit the author marked MUST-display (IIIF `requiredStatement`, `model.ts` RightsFields doc) never renders in the embed | Not addressed in ADRs 0019-0022. Distinct from the narrative/reading scoping: this is a rights-VISIBILITY gap — IIIF treats `requiredStatement` as a must-display obligation, so dropping it in the embed is the highest-stakes of the incidental drops | Surplus (rights-visibility concern) | **pursue** (user, 2026-07-17 — folded into parity, flagged must-ship) | Covered by the parity spec — the sections row's prompt names this the non-negotiable piece: the attribution/license footer ships first however the rest is phased. |
