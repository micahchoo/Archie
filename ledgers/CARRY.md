...
| **working** `libraryToWorking` (`publish/working.ts:174-200`) — Library→working (**the LIVE drop**) | `cover`, `format`, `originalName` — recovered on import (`cover` via exhibits.json/loadLibrary; `format` via objectsFromManifest) then dropped here → import a covered `.archie.zip` + republish ⇒ covers the viewer renders (`Gallery.svelte:80-81`) VANISH | FIXED — carry `cover`/`format`/`originalName` into the new working slots; `_libraryExhibitCarry`/`_libraryObjectCarry` sentinels keyed on `keyof Exhibit`/`keyof AObject`; `bakeTiles` named `{drop}` | `<c7>` | ✅ working.test.ts round-trip (cover/format/originalName survive Library→Working→Library); 749 render-core, 270 studio, svelte-check 0 err |
