...
- **Write (site.ts):** `archie.json` carries a `generation` string and is moved to the **LAST** write in
  ...
  defaults to `String(opts.publishedAt ?? Date.now())` and is overridable via `opts.generation` (so the
  bake agent / tests pin it deterministically); self-contained — no caller is required to supply it.
