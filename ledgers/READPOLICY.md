...
- **`readExhibitTree` degrades per-layer, not per-exhibit.** Each *optional* authored layer (readings.json,
  ...
  optional-with-distinction path (404 → empty object; 5xx/torn → partial). ExhibitView renders a visible
  "some notes couldn't load" indicator when `incomplete`.
...
| all | **25a** torn manifest.json | buildImageIndex **silently omits exhibit** (slugs=["a"]); loadLibrary **hard-throws** — one file, two policies | torn (failed) manifest → propagate loud on BOTH; genuinely-absent manifest → omit (buildImageIndex) | `<rp3>` | pass |
