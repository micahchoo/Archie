...
| **19d** DAG cycle (`linearHead` `heads.length===0` fallback) | a cycle is corruption → throw, not a silent guess | `linearHead` **silently returns the last version** (`?? versions[last]`) on a pure cycle — no head exists yet it hands one back | FIXED — `heads.length===0` ⟺ every version referenced as parent ⟺ cycle → throw a corruption error; removed the `?? versions[last]` fallback (dead code for valid DAGs) | `<fix3>` | ✅ log.test.ts 19d (cycle throws /cyclic\|corrupt/); 746 render-core all green |
