...
| **19c** duplicated page / duplicate revs (`fromHistory` vs `serialize`'s `dedupe`) | `fromHistory` dedupes by `rev` like `serialize.ts:50-60`; a duplicated page does not spawn plural heads | `fromHistory([page,page])` returns **2** records for a 1-record page (no dedupe) → duplicate `rev` survives → spurious `plural heads` throw at `log.ts:49` on the next edit | FIXED — dedupe by `rev` in `fromHistory` (first-seen `Set<RevId>`, mirrors serialize); distinct revs sharing (logicalId, version) — a genuine merge pair — preserved | `<fix2>` | ✅ deserialize.test.ts 19c (dup page → 1 head; concurrent pair preserved) |
