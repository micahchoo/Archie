...
<style>
  .lb-scrim { position: fixed; inset: 0; background: rgba(10,14,39,0.92); z-index: 60; }
  .lb {
  ...
  .lb-stage img, .lb-stage video { max-width: 100%; max-height: 72vh; object-fit: contain; border-radius: 0; background: var(--surface-canvas); box-shadow: var(--shadow-pixel); }

  .audio-stage { display: flex; flex-direction: column; align-items: center; gap: var(--space-4); width: min(40rem, 80vw); }
  .audio-stage .now { font-family: var(--font-ui, monospace); font-size: 0.7rem; font-weight: 600; letter-spacing: 0.14em; text-transform: uppercase; color: var(--accent); }
  ...
  .nav {
    ...
    border: var(--border-pixel, 2px) solid var(--accent);
    font-family: var(--font-ui, monospace); font-size: 1.5rem; line-height: 1;
    box-shadow: var(--shadow-pixel);
  }
  .nav:hover { background: var(--accent-muted); border-color: var(--accent); color: var(--accent); }
  .nav.prev { left: var(--space-3); }
  .nav.next { right: var(--space-3); }
  .counter {
    position: absolute; bottom: var(--space-3); left: 50%; transform: translateX(-50%);
    font-family: var(--font-mono, monospace); font-size: 0.72rem; letter-spacing: 0.1em; text-transform: uppercase;
    color: var(--ink-canvas-secondary);
    background: var(--surface-canvas-overlay); padding: 2px 10px; border-radius: 0;
    border: var(--border-pixel, 2px) solid var(--border-canvas-emphasis);
  }

  .lb-text {
    flex-shrink: 0; max-height: 24vh; overflow: auto;
    font-family: var(--font-body, serif); font-size: 1.0625rem; line-height: 1.6; color: var(--ink-canvas-primary);
    background: var(--surface-canvas-overlay); border-left: var(--border-pixel-bold, 4px) solid var(--accent);
    border-radius: 0; box-shadow: var(--shadow-pixel); padding: var(--space-3) var(--space-4);
  }
  .lb-text :global(p) { margin: 0 0 var(--space-2); }
  .lb-text :global(p:last-child) { margin-bottom: 0; }
  .lb-text :global(a) { color: var(--accent); }
