...
<style>
  ...
  header { display: flex; align-items: center; gap: var(--space-4); padding: var(--space-4) var(--space-6); border-bottom: var(--border-pixel) solid var(--border-canvas-emphasis); }
  .back { font-family: var(--font-ui); font-size: var(--text-ui-sm); text-transform: uppercase; letter-spacing: 0.1em; cursor: pointer; padding: var(--space-1) var(--space-3); background: transparent; color: var(--accent); border: var(--border-pixel) solid var(--accent); border-radius: 0; }
  .back:hover { color: var(--ink-on-accent); background: var(--accent-muted); border-color: var(--accent); }
  .titles .eyebrow { margin: 0; }
  .titles h1 { margin: 2px 0 0; font-family: var(--font-display); font-weight: 800; font-size: 1.9rem; line-height: 1.05; color: var(--ink-canvas-primary); text-shadow: var(--text-shadow-hero); }
  .titles .intent { margin: 4px 0 0; font-family: var(--font-ui); font-size: var(--text-ui-sm); text-transform: uppercase; letter-spacing: 0.1em; color: var(--ink-canvas-secondary); }
  .spacer { flex: 1; }
  .chip { font-family: var(--font-ui); font-size: var(--text-ui-sm); text-transform: uppercase; letter-spacing: 0.1em; cursor: pointer; padding: var(--space-1) var(--space-3); background: var(--surface-canvas-overlay); color: var(--ink-canvas-primary); border: var(--border-pixel) solid var(--border-canvas-emphasis); border-radius: 0; }
  ...
  .viewtoggle { display: inline-flex; border: var(--border-pixel) solid var(--border-canvas-emphasis); border-radius: 0; overflow: hidden; }
  .viewtoggle button { font-family: var(--font-ui); font-size: var(--text-ui-sm); text-transform: uppercase; letter-spacing: 0.1em; cursor: pointer; padding: 6px var(--space-3); background: transparent; color: var(--ink-canvas-secondary); border: none; } /* 6px v-pad -> 25px hit box (Fitts) */
  ...
  .viewport { position: relative; flex: 1; min-height: 0; overflow: hidden; cursor: grab; touch-action: none; background:
    radial-gradient(circle at 1px 1px, var(--border-canvas-emphasis) 1px, transparent 0) 0 0 / var(--grid-size) var(--grid-size); } /* neon dot-grid = a pixel surface, not a page */
  ...
  .edges { position: absolute; inset: 0; pointer-events: none; box-shadow: inset 0 0 64px 10px var(--void); }
  /* Gesture legend — names the two non-obvious gestures, prominently, top-centre. */
  .canvas-legend { position: absolute; top: var(--space-4); left: 50%; transform: translateX(-50%); display: flex; align-items: center; gap: var(--space-2); padding: var(--space-2) var(--space-4); font-family: var(--font-ui); font-size: var(--text-ui-sm); text-transform: uppercase; letter-spacing: 0.1em; color: var(--ink-canvas-primary); background: var(--surface-canvas-raised); border: var(--border-pixel) solid var(--border-canvas-emphasis); border-radius: 0; box-shadow: var(--shadow-pixel); pointer-events: none; }
  ...
  .plate { display: flex; flex-direction: column; gap: var(--space-2); width: 13rem; cursor: pointer; text-align: left; padding: var(--space-3); background: var(--surface-canvas-raised); border: var(--border-pixel) solid var(--border-canvas-emphasis); border-radius: 0; box-shadow: var(--shadow-pixel); transition: border-color 140ms ease, transform 140ms ease, box-shadow 140ms ease; }
  .plate:hover { border-color: var(--accent); transform: translate(2px, 2px); box-shadow: var(--shadow-pixel-cyan); }
  .plate .order { font-family: var(--font-mono); font-size: var(--text-ui-xs); text-transform: uppercase; color: var(--accent-3); }
  .frame { position: relative; aspect-ratio: 4 / 3; border-radius: 0; overflow: hidden; background: var(--surface-canvas-overlay); border: var(--border-pixel) solid var(--border-canvas); display: flex; align-items: center; justify-content: center; }
  ...
  .caption .lbl { font-family: var(--font-display); font-size: 1.15rem; font-weight: 700; line-height: 1.1; color: var(--ink-canvas-primary); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
  .caption .cnt { font-family: var(--font-mono); font-size: 0.68rem; text-transform: uppercase; letter-spacing: 0.1em; color: var(--ink-canvas-secondary); }
  .plate.add { border-style: dashed; justify-content: center; }
  .add-frame { border: var(--border-pixel) dashed var(--border-canvas-emphasis); }
  ...
  .canvas-legend .lead { color: var(--accent-3); }
  ...
  .dropstart { width: 0; align-self: stretch; border-radius: 0; transition: width 120ms ease; }
  .dropstart.armed { width: 1.5rem; border: var(--border-pixel) dashed var(--border-canvas-emphasis); }
  .dropstart.over { border-color: var(--accent); border-style: solid; box-shadow: 4px 0 0 var(--accent); }

  .zoomctl { position: absolute; bottom: var(--space-5); right: var(--space-5); display: flex; gap: var(--border-pixel); background: var(--border-canvas-emphasis); border: var(--border-pixel) solid var(--border-canvas-emphasis); border-radius: 0; box-shadow: var(--shadow-pixel); overflow: hidden; }
  .zoomctl button { font-family: var(--font-display); font-weight: 700; font-size: 1.05rem; cursor: pointer; min-width: 2.25rem; padding: var(--space-2) var(--space-2); background: var(--surface-canvas-raised); color: var(--ink-canvas-primary); border: none; }
  .zoomctl .fit { font-family: var(--font-ui); text-transform: uppercase; letter-spacing: 0.1em; font-size: var(--text-ui-sm); }
  .zoomctl button:hover { color: var(--ink-on-accent); background: var(--accent); }
  .zoomctl .pct { display: inline-flex; align-items: center; justify-content: center; min-width: 3rem; font-family: var(--font-mono); font-size: var(--text-ui-xs); color: var(--accent); background: var(--surface-canvas-raised); }
  .hint { position: absolute; bottom: var(--space-5); left: var(--space-6); margin: 0; font-family: var(--font-ui); font-size: var(--text-ui-xs); text-transform: uppercase; letter-spacing: 0.1em; color: var(--ink-canvas-muted); pointer-events: none; }

  /* 1b list fallback. */
  .list-hint { max-width: 48rem; margin: var(--space-6) auto 0; padding: 0 var(--space-6); font-family: var(--font-ui); font-size: var(--text-ui-sm); text-transform: uppercase; letter-spacing: 0.1em; color: var(--ink-canvas-secondary); }
  ...
  .list li.dropstart-row { height: 0; margin: 0; padding: 0; border-radius: 0; transition: height 120ms ease; }
  ...
  .list li button { display: flex; flex: 1; align-items: center; gap: var(--space-4); text-align: left; cursor: pointer; padding: var(--space-3); background: var(--surface-canvas-raised); border: var(--border-pixel) solid var(--border-canvas-emphasis); border-radius: 0; box-shadow: var(--shadow-pixel); color: inherit; transition: border-color 140ms ease, transform 140ms ease, box-shadow 140ms ease; }
  .list li button:hover { border-color: var(--accent); transform: translate(2px, 2px); box-shadow: var(--shadow-pixel-cyan); }
  .list li.end.over button { border-color: var(--accent); border-style: solid; color: var(--accent); }
  .li-order { font-family: var(--font-mono); font-size: var(--text-ui-xs); color: var(--accent-3); min-width: 1.5rem; }
  .li-thumb { width: 3rem; height: 2.25rem; border-radius: 0; border: var(--border-pixel) solid var(--border-canvas); background: var(--surface-canvas-overlay) center/cover; display: flex; align-items: center; justify-content: center; }
  .li-thumb .glyph { color: var(--accent-2); }
  .li-lbl { flex: 1; font-family: var(--font-display); font-size: 1.2rem; font-weight: 700; color: var(--ink-canvas-primary); }
  .li-cnt { font-family: var(--font-mono); font-size: 0.7rem; text-transform: uppercase; letter-spacing: 0.1em; color: var(--ink-canvas-secondary); }
  .li-add { font-family: var(--font-ui); font-size: var(--text-ui-sm); text-transform: uppercase; letter-spacing: 0.1em; color: var(--accent); background: none; border: var(--border-pixel) dashed var(--accent); border-radius: 0; padding: var(--space-3); cursor: pointer; width: 100%; }
  .li-add:hover { background: var(--accent-muted); color: var(--ink-on-accent); }
</style>
