...
<style>
  .scrim { position: fixed; inset: 0; z-index: 100; display: flex; align-items: center; justify-content: center; padding: var(--space-6); background: rgba(10, 14, 39, 0.72); }
  .sheet {
    width: min(40rem, 100%); max-height: 86vh; overflow-y: auto; box-sizing: border-box;
    background: var(--surface-paper); color: var(--ink-paper-primary);
    border: var(--border-pixel) solid var(--border-paper-emphasis); border-radius: var(--radius-md);
    box-shadow: var(--shadow-pixel-strong); padding: var(--space-6);
  }
  header { display: flex; align-items: baseline; justify-content: space-between; margin-bottom: var(--space-4); }
  header h2 { margin: 0; font-family: var(--font-display); font-size: 1.5rem; font-weight: 800; color: var(--ink-paper-primary); }
  ...
  section h3 { margin: 0 0 var(--space-2); font-family: var(--font-ui); font-size: var(--text-ui-xs); font-weight: 700; letter-spacing: 0.16em; text-transform: uppercase; color: var(--accent); }
  ...
  /* Square Space Mono navy key caps — pixel chips */
  kbd { font-family: var(--font-mono); font-size: 0.72rem; line-height: 1.4; color: var(--ink-canvas-primary); background: var(--surface-canvas-overlay); border: var(--border-pixel) solid var(--border-canvas-emphasis); border-radius: var(--radius-sm); padding: 1px var(--space-2); text-transform: uppercase; }
  .foot { margin: var(--space-5) 0 0; font-family: var(--font-ui); font-size: var(--text-ui-xs); letter-spacing: 0.1em; text-transform: uppercase; color: var(--ink-paper-muted); }
</style>
