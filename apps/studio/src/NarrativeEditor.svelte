...
<style>
  ...
  .spine { display: flex; flex-direction: column; gap: var(--space-3); margin: 0 0 var(--space-4); padding: var(--space-3); background: var(--surface-paper-card); border: var(--border-pixel) solid var(--border-paper); border-top: var(--border-pixel-bold) solid var(--accent); border-radius: 0; box-shadow: var(--shadow-pixel); }
  .spine > header .eyebrow { margin: 0; font-family: var(--font-ui); font-size: var(--text-ui-xs, 0.7rem); font-weight: 700; letter-spacing: 0.1em; text-transform: uppercase; color: var(--accent); }
  ...
  .card { display: flex; gap: var(--space-2); padding: var(--space-2); background: var(--surface-paper); border: var(--border-pixel) solid var(--border-paper); border-left: var(--border-pixel-bold) solid transparent; border-radius: 0; opacity: 0.55; transition: opacity 120ms ease, border-color 120ms ease, box-shadow 120ms ease; }
  .card.here { opacity: 1; border-left-color: var(--accent); }
  .card.framing { opacity: 1; border-left-color: var(--accent); box-shadow: var(--shadow-pixel-cyan); }

  .ord { display: flex; flex-direction: column; align-items: center; gap: var(--space-1); }
  .ord .n { font-family: var(--font-display); font-weight: 700; font-size: 0.85rem; color: var(--accent); }
  .mv { display: flex; flex-direction: column; gap: 2px; }
  .mv button { cursor: pointer; font-size: 0.55rem; line-height: 1; padding: 2px 4px; background: none; border: var(--border-pixel) solid var(--border-paper-emphasis); border-radius: 0; color: var(--ink-paper-secondary); }
  ...
  .title { font-family: var(--font-display); font-size: 1.05rem; font-weight: 700; padding: var(--space-1) var(--space-2); background: var(--surface-paper); color: var(--ink-paper-primary); border: var(--border-pixel) solid var(--border-paper); border-radius: 0; }
  .shows { margin: 0; display: flex; align-items: center; gap: var(--space-2); flex-wrap: wrap; }
  .on-obj { font-family: var(--font-ui); font-size: var(--text-ui-xs, 0.7rem); font-weight: 600; letter-spacing: 0.1em; text-transform: uppercase; color: var(--ink-paper-secondary); }
  .move-here { cursor: pointer; font-family: var(--font-ui); font-size: 0.65rem; letter-spacing: 0.08em; text-transform: uppercase; padding: 1px var(--space-2); background: none; color: var(--accent); border: var(--border-pixel) dashed var(--accent-muted); border-radius: 0; }
  .move-here:hover { border-color: var(--accent); border-style: solid; background: var(--accent-muted); }
  ...
  .prose-wrap .cite { align-self: flex-end; display: inline-flex; align-items: center; gap: var(--space-1); cursor: pointer; background: none; border: none; padding: 0; font-family: var(--font-ui); font-size: var(--text-ui-xs, 0.7rem); font-weight: 600; letter-spacing: 0.08em; text-transform: uppercase; color: var(--accent); }
  .prose-wrap .cite:hover { color: var(--accent-hover); }
  .prose-wrap .cite kbd { font-family: var(--font-mono); font-size: 0.6rem; color: var(--ink-paper-muted); background: var(--surface-paper-hover); border: var(--border-pixel) solid var(--border-paper); border-radius: 0; padding: 0 var(--space-1); }
  .prose { font-family: var(--font-body); font-size: 0.9rem; line-height: 1.6; padding: var(--space-2); background: var(--surface-paper); color: var(--ink-paper-primary); border: var(--border-pixel) solid var(--border-paper); border-radius: 0; resize: vertical; }
  ...
  .cam { font-family: var(--font-mono); font-size: 0.7rem; letter-spacing: 0.04em; color: var(--ink-paper-muted); }
  .cam.set { color: var(--accent); }
  .framing-now { font-family: var(--font-ui); font-size: var(--text-ui-xs, 0.7rem); font-weight: 600; letter-spacing: 0.08em; text-transform: uppercase; color: var(--accent); }
  .set-cam, .cancel { cursor: pointer; font-family: var(--font-ui); font-size: 0.7rem; letter-spacing: 0.08em; text-transform: uppercase; padding: 2px var(--space-2); border-radius: 0; }
  .set-cam { background: none; color: var(--accent); border: var(--border-pixel) solid var(--accent-muted); }
  .set-cam:hover { border-color: var(--accent); background: var(--accent-muted); }
  .cancel { background: none; color: var(--ink-paper-secondary); border: var(--border-pixel) solid var(--border-paper-emphasis); }
  ...
  /* Primary CTA — full pixel-btn treatment: Tektur uppercase, navy ink on cyan, hard cascade shadow,
     press-down on hover (no class hook in markup, so the rules are replicated here). */
  .add { align-self: flex-start; cursor: pointer; font-family: var(--font-display); font-size: 0.8125rem; font-weight: 700; letter-spacing: 0.08em; text-transform: uppercase; padding: var(--space-1) var(--space-3); background: var(--accent); color: var(--ink-on-accent); border: var(--border-pixel-bold) solid var(--accent); border-radius: 0; box-shadow: var(--shadow-pixel-btn); transition: transform 80ms steps(2), box-shadow 80ms steps(2); }
  .add:hover:not(:disabled) { transform: translate(2px, 2px); box-shadow: var(--shadow-pixel-btn-active); }
  .add:disabled { background: var(--accent-muted); color: var(--ink-paper-muted); border-color: var(--accent-muted); box-shadow: none; cursor: default; }
  .from-note { font-family: var(--font-ui); font-size: 0.75rem; letter-spacing: 0.08em; text-transform: uppercase; padding: var(--space-1) var(--space-2); cursor: pointer; background: var(--surface-paper); color: var(--ink-paper-secondary); border: var(--border-pixel) dashed var(--border-paper-emphasis); border-radius: 0; }
  .from-note:hover { border-color: var(--accent); border-style: solid; color: var(--accent); }
</style>
