 ...
 */
  ...
  // UX-CRITIQUE O1+O2: the ONE "this tab cannot write" condition. tabReadOnly feeds the read-only
  // banner and every SafetyState mount (library / overview / editor) so the header can never offer
  // "⚠ Retry save" in a tab whose writes the save-queue gate refuses by design; canWriteNow is its
  // complement, shared by the overview's affordance gating and the destructive keyboard paths so
  // none can drift. Pre-claim (otherTabActive still false) it reads writable — no read-only flash
  // at boot; the save-queue's writer gate still refuses that brief window as before.
  const tabReadOnly = $derived(writerLock.otherTabActive && !writerLock.canWrite);
  const canWriteNow = $derived(!tabReadOnly);
