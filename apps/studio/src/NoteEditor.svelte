<script lang="ts">
 /**
  * @surface {popover}
  * @composes {WADM note form}
  * @variants {region scope, whole-object scope; AV time fieldset visible/hidden; stacked notes co-location stepper}
  * @constraint {popover is NOT a modal — dismiss on Done/close; click-away dismisses via App's popover chrome}
  */
 ...
 const isRegion = $derived(
  typeof sel.target !== "string" &&
   !!(sel.target as { selector?: unknown }).selector,
 );
 ...
 function parseMMSS(v: string): number {
  ...
  if (t.includes(":")) {
   const [m, s] = t.split(":");
   return (parseInt(m || "0", 10) || 0) * 60 + (parseFloat(s || "0") || 0);
  }
...
>
 <h3>
  Edit note{#if showAttribution && you}<span class="attribution"
    >{attributionChip(sel, you)}</span
   >{/if}
 </h3>
 {#if coLocatedCount > 1}
  ...
  <div class="stack-nav" role="group" aria-label="Stacked notes at this spot">
   <button
    type="button"
    onclick={() => cycleCoLocated(-1)}
    title="Previous note at this spot"
    aria-label="Previous note here">‹</button
   >
   <span>{coLocatedIndex + 1} of {coLocatedCount} here</span>
   <button
    type="button"
    onclick={() => cycleCoLocated(1)}
    title="Next note at this spot"
    aria-label="Next note here">›</button
   >
  </div>
 {/if}
 <label>
  <span class="field-head"
   >Comment<button
    type="button"
    class="cite"
    onclick={() => void requestCite(citeIntoComment)}
    title="Cite a note, object, or exhibit (⌘K) — search by text or browse by image"
    ><span aria-hidden="true">¶</span> Cite <kbd>⌘K</kbd></button
   ></span
  >
  <textarea
   bind:this={commentEl}
   ...
   value={comment}
   onchange={(e) =>
    applyForm((e.currentTarget as HTMLTextAreaElement).value, tags)}
  ></textarea>
 </label>
 {#if trange}
  <fieldset class="time">
   ...
   <label class="t"
    >Start<input
     ...
     inputmode="numeric"
     placeholder="m:ss"
     value={fmtMMSS(trange.start)}
     onchange={(e) =>
      applyTime(
       parseMMSS((e.currentTarget as HTMLInputElement).value),
   ...
   <label class="t"
    >End<input
     ...
     inputmode="numeric"
     placeholder="m:ss"
     value={fmtMMSS(trange.end ?? trange.start)}
     onchange={(e) =>
      applyTime(
       trange.start,
       parseMMSS((e.currentTarget as HTMLInputElement).value),
 ...
 <div class="scope-field">
  ...
  <div class="scope-row">
   <span class="scope-now"
    >{isRegion
     ? "A region of this object"
     : "The whole object (no region)"}</span
   >
   {#if isRegion}
    <button
     type="button"
     class="cite"
     onclick={() => setNoteScope("whole")}
     title="Drop the region — make this note apply to the whole object"
     ><span aria-hidden="true">▣</span> Make whole-object</button
    >
    <button
     type="button"
     class="cite"
     onclick={() => setNoteScope("region")}
     title="Re-draw this note's region — draw a new box or outline on the object"
     ><span aria-hidden="true">▭</span> Redraw bounds</button
    >
   {:else}
    <button
     type="button"
     class="cite"
     onclick={() => setNoteScope("region")}
     title="Give this note a region — draw a box or outline on the object"
     ><span aria-hidden="true">▭</span> Draw a region</button
    >
   {/if}
  </div>
 </div>
 <label
  >Tags (comma-separated)<input
   value={tags}
   onchange={(e) =>
    applyForm(comment, (e.currentTarget as HTMLInputElement).value)}
  /></label
 >
 <label
  >Reading
  <select
   value={reading ?? ""}
   onchange={(e) =>
    setNoteReading((e.currentTarget as HTMLSelectElement).value || null)}
  >
   <option value="">{readingBadge("base", readingIds)} — General notes —</option
   >
   {#each currentReadings as r (r.id)}<option value={r.id}
     >{readingBadge(r.id, readingIds)} {r.name}</option
    >{/each}
  </select>
 </label>
 <label
  ...
  <select
   value={emphasis}
   onchange={(e) =>
    setNoteEmphasis((e.currentTarget as HTMLSelectElement).value as Emphasis)}
   title="How much a mark stands out — its weight, not its colour."
  >
 ...
 <div class="wadm-actions">
  ...
  <button type="button" class="del" onclick={() => ondelete(editing)}
   >Delete note</button
  >
 </div>
</form>
...
<style>
 ...
 .scope-field .field-head {
  font-family: var(--font-ui);
  font-size: 0.7rem;
  font-weight: 400;
  letter-spacing: 0.14em;
  text-transform: uppercase;
  color: var(--ink-paper-muted);
 }
 .scope-row {
  display: flex;
