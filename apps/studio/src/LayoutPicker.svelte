...
<div class="scrim" role="presentation" onclick={() => onclose()}>
  <div class="sheet" role="dialog" aria-label="Choose how this exhibit is read" onclick={(e) => e.stopPropagation()}>
    <header>
      <h2 class="pixel-bracket">How should this exhibit be read?</h2>
...
<style>
  /* Pixel-arcade intent sheet over the dark header — choosing the form of a finished publication.
     The active choice + its diagram region are inked neon-cyan (active state). */
  .scrim { position: fixed; inset: 0; z-index: 60; display: flex; justify-content: center; align-items: flex-start; padding-top: 10vh; background: rgba(10, 14, 39, 0.72); }
  .sheet {
  ...
  header h2 { font-family: var(--font-display); font-size: 1.5rem; font-weight: 800; margin: 0 0 var(--space-2); color: var(--accent); text-shadow: var(--text-shadow-hero); }
  ...
  .fam-name { font-family: var(--font-ui); font-size: var(--text-ui-xs); font-weight: 700; letter-spacing: 0.16em; text-transform: uppercase; color: var(--accent); }
  ...
  .future { margin: var(--space-2) 0 0; font-family: var(--font-ui); font-size: var(--text-ui-xs); line-height: 1.6; color: var(--ink-paper-muted); padding-left: var(--space-3); border-left: var(--border-pixel) solid var(--border-paper); }
  ...
  button {
    ...
    border: var(--border-pixel) solid var(--border-paper); border-left: var(--border-pixel-bold) solid transparent; border-radius: 0;
    transition: background 120ms ease, border-color 120ms ease;
  }
  button:hover { background: var(--surface-paper-hover); border-left-color: var(--accent); }
  button.active { background: var(--accent-muted); border-color: var(--border-paper-emphasis); border-left-color: var(--accent); }

  .diagram { flex-shrink: 0; width: 64px; height: 44px; }
  .diagram svg { width: 64px; height: 44px; }
  .frame { fill: none; stroke: var(--ink-canvas-secondary); stroke-width: 2; }
  .frame.fill { fill: var(--border-canvas); }
  .rule { stroke: var(--ink-canvas-secondary); stroke-width: 2; stroke-linecap: square; }
  ...
  .name { font-family: var(--font-display); font-size: 1.2rem; font-weight: 800; line-height: 1; display: flex; align-items: baseline; gap: var(--space-2); }
  .now { font-family: var(--font-mono); font-size: 0.6rem; letter-spacing: 0.14em; text-transform: uppercase; color: var(--accent-3); }
  ...
  .note { margin: var(--space-4) 0 0; padding: var(--space-3) var(--space-4); border: var(--border-pixel) dashed var(--border-paper-emphasis); border-radius: 0; font-family: var(--font-ui); font-size: 0.78rem; line-height: 1.6; color: var(--ink-paper-muted); }
</style>
