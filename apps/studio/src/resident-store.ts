...
async function desktopResidentFs(): Promise<Filesystem> {
  return (desktopReady ??= (async () => {
    ...
    await generationGuard.adoptBaseline(folder); // Phase 5b: adopt/stamp the generation baseline for this session
    return folder;
  })());
}
...
// resident folder DURING a session. Uses the shared generation-guard protocol (generation-guard.ts):
// Archie stamps the folder, and before a working-store commit re-reads it — a mismatch means something
// else wrote, so refuse rather than blind-overwrite Archie's (now stale) copy over the external change.
// The baseline is ADOPTED at mount (the first commit can be incremental, so the pre-existing tree is
// trusted from commit #1 — the resident half of the baseline decision documented in generation-guard.ts).
// isTauri-gated; on WEB it is a pure no-op (OPFS is serialized across tabs by navigator.locks — there is
// no cross-process writer).

/** The session generation guard (baseline adopted at mount, restamped after each successful commit). */
...
export async function residentExternallyChanged(): Promise<boolean> {
  ...
  if (!fs) return false;
  return generationGuard.verify(fs);
}
...
export async function restampResident(): Promise<void> {
  ...
  const fs = await residentRootFs();
  if (!fs) return;
  await generationGuard.restamp(fs);
}
