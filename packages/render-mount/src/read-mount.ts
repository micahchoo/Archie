...
export async function createReadOnlyMount(
 ...
 /** Split the list: region shapes → the SVG overlay; whole-object notes → clickable frames whose
  *  onActivate emits that record id through the SAME onSelect path region clicks use. An object may
  *  carry multiple whole-object notes (a bare-canvas note AND a region-with-wholeObject-override note,
  *  per ADR-0018). The frame is a single object-spanning border (frame-overlay.ts draw replaces), so
  *  the LAST whole-object note wins the frame click; ALL remain in the annotations list for sidebar
  *  listing and fitBounds. The full viewer (Reader.svelte) keeps all in list/detail identically. */
 const applyAnnotations = (annotations: W3CAnnotation[]): void => {
  ...
  for (const whole of wholeObjects) {
  ...
  if (wholeObjects.length === 0) frame.clear();
 };
