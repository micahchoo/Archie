...
- **REMAINING — Phase 2 staging UX** (per the strategy doc; invented UX → **comprehension-gated**, philosophy #6): lead with the keystone **matched-pair "leads now / back to grid" cues** (0→1 and last→0 section transitions — today nothing tells the author their exhibit's front door changed); then the recessed always-present narrative card, overview "Start the narrative" invitation + spine surface, object-switcher in the object view, dimming explainer on object-switch, reorder path preview, and the **grid-index escape from inside the read** (breaks today's whole-exhibit takeover at `ExhibitView.svelte:196`). All copy is verbatim in the strategy doc.
- **Should-fixes (non-blocking):** SF-2 — `store.ts:77-97` dead duplicate `ExhibitMeta`/etc. block (TS2484, pre-existing); delete it + add a Studio `svelte-check` script (the missing type-gate is the root cause of the persistent false-positive `.svelte` diagnostics: `onclick`/`ondragover` "not in HTMLProps", ".svelte has no default export", "Unexpected token" in templates — `vite build` is the real gate).
- **⚠ COMMIT BOUNDARY (SF-6):** the working tree COMMINGLES a second unrelated in-flight feature (cite-cards link-scent / publish-base `WORKING_IRI_BASE` rework — `cite-cards.ts`, `ProseCites.svelte`, `published-base.ts`, regenerated `public/published/**`). **Do NOT `git add -A`** — stage only the narrative-layer files above so a commit doesn't straddle two ADRs.
